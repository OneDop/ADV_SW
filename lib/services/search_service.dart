...
class SearchService {
  ...
  Future<List<SearchUserResult>> searchUsersByName(String name) async {
    try {
      final response = await _apiClient.get('/search/users', queryParameters: {'name': name});
  ...
  Future<List<SearchUserResult>> searchUsersBySkill(int skillId, {ExperienceLevel? experienceLevel}) async {
    try {
      ...
      final response = await _apiClient.get('/search/users/skill', queryParameters: params);
      final List<dynamic> data = response.data;
      return data.map((json) => SearchUserResult.fromJson(json)).toList();
    } catch (e) {
      rethrow;
  ...
  Future<List<SearchUserResult>> searchUsers({String? name, List<int>? skillIds}) async {
    try {
      ...
      if (skillIds != null && skillIds.isNotEmpty) params['skillIds'] = skillIds.join(',');
      final response = await _apiClient.get('/search/users/advanced', queryParameters: params);
      final List<dynamic> data = response.data;
      return data.map((json) => SearchUserResult.fromJson(json)).toList();
    } catch (e) {
      rethrow;
